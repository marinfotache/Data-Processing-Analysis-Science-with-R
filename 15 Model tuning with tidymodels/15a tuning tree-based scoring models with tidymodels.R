...
test__rf |> collect_metrics(type = "wide") 

# adding some other performance metrics:
compute_metrics(test__rf, metric_set(rmse, mae, rsq, ccc))


set.seed(1234)
test__xgb <- final_wf_xgb |> last_fit(splits) 
test__xgb |> collect_metrics() 
compute_metrics(test__xgb, metric_set(rmse, mae, rsq, ccc))
