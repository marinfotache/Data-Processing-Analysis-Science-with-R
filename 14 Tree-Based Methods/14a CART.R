...
set.seed(123)
states_rpart_1_unpruned <- rpart::rpart (Murder ~ ., data = main_train__states, 
     method = 'anova', control = rpart.control(cp = 0))   ## cp = 0
...
as.tibble(states_rpart_1_unpruned$cptable) %>%
     top_n(1, -xerror) 
...
set.seed(789)
states2_rpart_1_unpruned <- rpart::rpart (Murder ~ ., data = main_train__states2, 
...
# predicting the test data sets
predict_insurance_rpart_2 <- predict(insurance_rpart_2, main_test__insurance)
predict_insurance_rpart_3 <- predict(insurance_rpart_3, main_test__insurance)
...
# predicting the test data sets
predict_states_rpart_caret <- predict(states_rpart_caret_train, main_test__states)
...
set.seed(123)
heart_rpart_1_unpruned <- rpart (AHD ~ ., data = main_train__Heart, 
...
#                           Build the unpruned tree
set.seed(123)
css_rpart_1_unpruned <- rpart (Status ~ ., data = main_train__cs_sanchez, 
     method = 'class', control = rpart.control(cp = 0))
